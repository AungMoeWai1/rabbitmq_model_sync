...
class TestRabbitMqConsumerController(TransactionCase):
    ...
    @patch('odoo.addons.rabbitmq_model_sync.models.rabbitmq_consumer_controller.ReconnectingAsyncAttendanceConsumer')
    ...
    @patch('odoo.addons.rabbitmq_model_sync.models.rabbitmq_consumer_controller.modules.registry.Registry.new')
    def test_process_rabbitmq_message(self, mock_registry_new):
        ...
        with patch('odoo.api.Environment', return_value=mock_env):
            ...
            body = b'{"foo": "bar"}'
            self.controller._process_rabbitmq_message(method, properties, body, 'test.model')
